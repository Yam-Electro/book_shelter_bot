...
Если `VLESS_URL` заполнен, контейнер `proxy` поднимает Xray и бот сначала пробует Telegram через SOCKS5. Если VLESS не поднимается (ошибка REALITY и т.п.), бот автоматически переходит на прямое подключение — это помогает, когда VPN уже включён на хосте.
...
Напишите название, автора или серию. Либо откройте `/catalog` (кнопка «📚 Каталог») — там только книги из уже скачанных zip.
